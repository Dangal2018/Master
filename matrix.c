...
ERROR_ID matrix_multiplication(_IN MATRIX* A, _IN MATRIX* B, _OUT MATRIX* C)
{
	...
	for (i = 0; i < A->rows; i++)
	{
		for (j = 0; j < B->columns; j++)		//cpu cache优化
		{
			sum = 0.0;
			for (k = 0; k < A->columns; k++)
			{	
				if (A->p[i * A->columns + k] == 0 || B->p[k * B->columns + j] == 0) continue; //如果有一个为0就不用计算了
...
Description: lambda * 矩阵A + 矩阵B = 矩阵C
Input: 矩阵A,矩阵B
Output: 矩阵C
Input_Output: 无
Return: 错误号
...
ERROR_ID matrix_addex(_IN MATRIX* A, _IN MATRIX* B, _OUT MATRIX* C,REAL lambda)
{
	...
	for (i = 0; i < rows; i++)
	{
		for (j = 0; j < columns; j++)
		{
			C->p[i * columns + j] = lambda * A->p[i * columns + j] + B->p[i * columns + j];
		}
	}
	return errorID;
}
/**********************************************************************************************
Function: matrix_addexp
Description: lambda * 矩阵A + 矩阵B = 矩阵C
Input: 矩阵A,矩阵B
Output: 矩阵C
Input_Output: 无
...
Author: Marc Pony(marc_pony@163.com)
***********************************************************************************************/
ERROR_ID matrix_addexp(_IN MATRIX* A, _IN MATRIX* B, _OUT MATRIX* C, REAL lambda)
{
	INDEX i, j;
	INTEGER rows, columns;
	ERROR_ID errorID = _ERROR_NO_ERROR;
